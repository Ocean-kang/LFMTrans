...
def get_retrieval_matrix(graph):
    if graph is None:
        return None, None, None
        
    top_1 = top_5 = top_10 = total = 0
    
    num_targets = graph.shape[1]
    ...
    return top_1/num_targets, top_5/num_targets, top_10/num_targets
...
if __name__ == "__main__":
    ...
    from load_feature import llama_features, mpnet_features, llama_unmean_features, mpnet_unmean_features
    datasets = ['cocostuff', '150', '847', 'voc20', 'voc20b', 'pc59', 'ImageNet-100', 'CIFAR-100']

    feature_dict = mpnet_features(datasets)
    feat_v_anchor = feature_dict["cocostuff"]['patch'].float().to(device)
    feat_t_anchor = feature_dict["cocostuff"]['mpnet'].float().to(device)

    feat_v_query = feature_dict["150"]['patch'].float().to(device)
    feat_t_query = feature_dict["150"]['mpnet'].float().to(device)

    tmp = linear_local_CKA(feat_v_anchor, feat_t_anchor, feat_v_query, feat_t_query[:100, :], device)
    # tmp = kernel_local_CKA(feat_v_anchor, feat_t_anchor, feat_v_query, feat_t_query, device)
    res1 = get_retrieval_matrix(tmp)
    print(res1)
    # res2 = linear_matching(feat_v_anchor, feat_t_anchor, feat_v_query, feat_t_query, tmp, device)
    # print(res2)
    breakpoint()
