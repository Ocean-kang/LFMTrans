...
def load_feature_llama_unmean(dataset):
    ...
    if dataset in ['cocostuff', '150', '847', 'voc20', 'pc59', 'ImageNet-100', 'CIFAR-100']:
        ...
        raise ValueError(f'Unmean features not available for dataset {dataset}')
    return ret_dict

def load_feature_mpnet(dataset):

    ret_dict ={}
    mpnet_path = Path(f'./feature/feat_mpnet_{dataset}_S2P0_27_28_29_8_12_26_30.pkl')
    dinov2_patchtoken_path = Path(f'./feature/feat_dinov2_patch_{dataset}_L.pkl')        
...
def load_feature_mpnet_unmean(dataset):
    ...
    if dataset in ['ImageNet-100', 'CIFAR-100']:
        mpnet_path_unmean = Path(f'./feature/feat_mpnet_{dataset}_S2P0_27_28_29_8_12_26_30_unmean.pkl')
        dinov2_patchtoken_unmean_path = Path(f'./feature/feat_synonym_dinov2_{dataset}_L.pkl')
